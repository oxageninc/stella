...
| Google Vertex AI | `vertex` | `VERTEX_ACCESS_TOKEN` | `gemini-3-pro` | project/region-scoped | Gemini via Vertex |
...
| OpenRouter | `openrouter` | `OPENROUTER_API_KEY` | `auto` | `https://openrouter.ai/api/v1` | OpenAI-compatible |
...
You can also pin the base URL (and an optional key) in
[`settings.json`](/docs/configuration/settings) under `providers.local` so you don't pass
`--base-url` every time.
