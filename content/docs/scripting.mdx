...
machine interface you can parse incrementally.

```bash
# One final JSON object
stella run "list the public API of the auth module" --output-format json

# Newline-delimited event stream, parsed as it arrives
stella run "fix the failing test" --output-format stream-json | while IFS= read -r line; do
