...
A server entry needs a `transport` discriminant (`stdio` or `http`). For a stdio server, `cmd` is the executable and `args` its arguments; Stella launches it and merges the tools it reports for the session.
