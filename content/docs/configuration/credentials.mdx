...
2. **Provider env var** — e.g. `ANTHROPIC_API_KEY`, plus aliases like `GOOGLE_API_KEY`
   for Gemini.
3. **`settings.json`** — `providers.<id>.api_key` (see
   [settings.json](/docs/configuration/settings)).
4. **`~/.config/stella/credentials.toml`** — the stored-keys file below.
5. **Interactive prompt** — on a TTY, Stella prompts for the key and saves it to
   `credentials.toml`, so you are only asked once.
...
These are read from the environment. See [Providers &amp; models](/docs/providers) for the
full matrix.
