...
| `<workspace>/.stella/store.db` | Local [telemetry &amp; store](/docs/telemetry) — embedded SQLite |
...
| Shell commands on lifecycle events | `settings.json` `hooks` key | [Hooks](/docs/tools/hooks) |
| Org-managed defaults for a fleet | `STELLA_MANAGED_SETTINGS` → a `settings.json` | [settings hierarchy](#the-settings-hierarchy) |
| MCP servers to connect | `<workspace>/.stella/mcp.toml` | [MCP](/docs/tools/mcp) |
| Custom script tools | `<workspace>/.stella/tools/*.toml` | [Custom tools](/docs/tools/custom-tools) |
| Per-tool permission gates | `settings.json` permission model | [Permissions](/docs/tools/permissions) |
...
**Point a Z.ai coding-plan subscription at its endpoint** (project scope):
