...
declared in a repository's project-scope file (`<workspace>/.stella/settings.json`) load
**only** when `STELLA_PROJECT_HOOKS=1` is set (a trust boundary, so a cloned repo can't run
commands on your machine unbidden); a stderr notice names any skipped project hooks.
