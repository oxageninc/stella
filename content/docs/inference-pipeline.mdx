...
```
triage → recall → plan → scope review → witness → execute → verify → judge → revise
```
...
a bounded number of revision turns (2 by default). After two *consecutive*
deterministic failures, Stella spends one judge call on **distress guidance**:
