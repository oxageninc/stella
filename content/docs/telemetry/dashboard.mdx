...
| **Executions** | Every run: kind, prompt, provider, model, outcome, cost, steps, tokens, tool calls, files touched |
| **Execution drill-down** | One run's per-step telemetry, every tool call, files touched, and the post-turn self-reflection |
| **Models** | Per provider/model: runs, resolve rate, cost, **$/resolved task**, tokens, average duration |
| **Tools** | The tool leaderboard: calls, errors, p50/max latency, bytes |
| **Files** | The most-touched files across runs |
| **Memory** | The memory plane: citations, reflections, promoted rules, skill usage |
| **MCP** | Per-server / per-tool MCP traffic |
| **Fleet** | The fleet ledger: runs, attempts, commits, spend |
