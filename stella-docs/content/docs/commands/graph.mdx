...
`stella graph` reads the code-graph index that [`stella init`](/docs/commands/init) builds at `<workspace>/.stella/codegraph.db`. Because it only reads that local index, it makes no model calls and needs no provider credential — it is instant and works on a plane.
...
<Callout type="info">
Run [`stella init`](/docs/commands/init) first. The graph is only as current as the last `init` — if you have moved or renamed a lot of code since, re-run `init` to rebuild `.stella/codegraph.db`.
</Callout>
