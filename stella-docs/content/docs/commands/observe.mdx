...
Serve the **Observatory**: a read-only web dashboard over the local telemetry in `.stella/private/store.db` and `.stella/private/fleet.db`. Nothing leaves your machine — the server binds to `127.0.0.1` only.
