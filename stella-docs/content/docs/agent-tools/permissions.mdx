...
- **Read-only tools** only observe your workspace and environment. They never change files, run commands, or perform side effects. These tools are marked `read_only: true`. The authoritative read-only set in the [built-in catalog](/docs/agent-tools) is `read_file`, `grep`, `glob`, `explorations`, `ci_status`, `search_issues`, and — once `stella init` has built the code graph — `graph_query`.
