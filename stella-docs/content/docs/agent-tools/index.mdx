...
All told: 35 always-on tools, up to 48 native tools with the `bash` opt-in, the code graph, media keys, and an issue backend, plus the six session tools — before any custom script tools or MCP servers merge in.
