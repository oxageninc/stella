...
These 39 tools register in every session — no configuration, no prerequisites. Grouped by what they touch:
...
All told: 39 always-on tools, up to 55 native tools with the `bash` and `web` opt-ins, the code graph, media keys, a search key, and an issue backend, plus the six session tools — before any custom script tools or MCP servers merge in.
