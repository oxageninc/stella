...
surfaced live in the TUI, included in `--output-format json` output, and persisted to the
local store — so you always have an auditable record of exactly what a run changed.
...
`stella --output-format json run` includes the full telemetry payload under `files_touched`, so a
script can assert on exactly what a run changed:

```bash
stella --output-format json run "add a --health flag and wire the readiness probe" \
...
```bash
touched=$(stella --output-format json run "$TASK" | jq -r '.files_touched[].path')
