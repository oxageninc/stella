...
[`stella stats` and the Observatory](/docs/telemetry/dashboard).
...
3. **Prompt caching is doing heavy lifting.** Stella keeps the system prefix
   ...
   *cached* rate (10–25× cheaper). That is why real runs cost far less than
   naive `input tokens × list price` math suggests. See
   [Context Engine](/docs/context-engine).

## Multipliers

- **[Goal mode](/docs/agent-modes/goal-mode)** repeats pipeline rounds until
