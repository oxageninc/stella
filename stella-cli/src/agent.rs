...
",
    ...
    let result = {
        ...
        // `goal.rs`) enforces via `HEADLESS_SCOPE_REVIEW_BYPASS`. Named
        // distinctly from the `interactive` tool set above (an
        // `InteractiveToolSet`, an unrelated thing that happens to share the
        // word).
        let stdio_approval_available =
        ...
        let pipeline_config = PipelineConfig {
            ...
            headless: !stdio_approval_available,
        ...
        let ports = PipelinePorts {
            ...
            approvals: if stdio_approval_available {
                &stdio_gate
            } else {
